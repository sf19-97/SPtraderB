...
const AdaptiveChart: React.FC<AdaptiveChartProps> = ({ 
  ...
  useEffect(() => {
    ...
    const chart = createChart(chartContainerRef.current, {
      ...
      timeScale: {
        ...
        barSpacing: 24, // Default bar spacing
      },
    });
  ...
  const loadDataAndMaintainView = async (timeframe: string, visibleRange: any, previousBarSpacing: number, previousTimeframe: string) => {
    ...
    try {
      ...
      if (visibleRange) {
        // CRITICAL: Adjust bar spacing FIRST to maintain visual consistency
        ...
        // NOW set the visible range after bar spacing is adjusted
        chartRef.current.timeScale().setVisibleRange({
