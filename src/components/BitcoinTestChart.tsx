...
const BitcoinTestChart: React.FC<BitcoinTestChartProps> = ({
  ...
  const updateCountdown = useCallback(() => {
    ...
    if (totalSeconds <= 5 && totalSeconds >= -5) {
      console.log(
        `[BitcoinChart] Countdown: ${totalSeconds}s, triggered: ${hasTriggeredNewCandleRef.current}, time: ${date.toISOString()}`
      );
    }
      ...
      setTimeout(() => {
        ...
        if (seriesRef.current) {
          ...
          if (currentData.length > 0) {
            ...
            console.log(
              '[BitcoinChart] Placeholder candle created precisely at',
    ...
    if (totalSeconds <= 0 && totalSeconds > -1 && !hasTriggeredNewCandleRef.current) {
      ...
      console.log(
        `[BitcoinChart] PLACEHOLDER TRIGGER at ${exactTime.toISOString()} (totalSeconds: ${totalSeconds}, ms: ${exactTime.getMilliseconds()})`
      );
      ...
      if (seriesRef.current) {
        ...
        if (currentData.length > 0) {
          ...
          seriesRef.current.setData(newData);
          console.log(
            '[BitcoinChart] Placeholder candle created at',
            new Date(newCandleTime * 1000).toLocaleTimeString()
          );
  ...
  const fetchChartData = async (
    ...
    try {
      ...
      console.log(
        `[BitcoinChart] Fetching ${tf} data from ${new Date(from * 1000).toISOString()} to ${new Date(to * 1000).toISOString()}`
      );
      ...
      if (response.data && response.data.length > 0) {
        ...
        if (chartData.length > 0) {
          ...
          console.log(
            '[BitcoinChart] Data is',
            hoursBehind.toFixed(1),
            'hours behind current time'
          );
        }
            ...
            (event) => {
              ...
              if (event.payload.timeframe === currentTimeframeRef.current) {
                ...
                fetchChartData(symbolRef.current, currentTimeframeRef.current, from, to)
                  ...
                  .catch((error) =>
                    console.error('[BitcoinChart] Failed to reload after update:', error)
                  );
              }
            }
          );
  ...
  useEffect(() => {
    ...
    console.log(
      `[BitcoinChart] Syncing periodic refresh - current: :${currentSecond}, next: :${nextTarget % 60}, delay: ${delaySeconds}s`
    );
            ...
            .then(({ data }) => {
              if (data.length > 0 && seriesRef.current) {
                ...
                if (currentData.length > 0 && data.length > 0) {
                  ...
                  if (existingIndex >= 0) {
                    ...
                    if (placeholderTime && placeholderTime > 0) {
                      ...
                      if (placeholderInCurrent && !placeholderInNew) {
                        ...
                        console.log(
                          '[BitcoinChart] Preserving placeholder candle at',
                    ...
                    if (preservedPlaceholder) {
                      ...
                      const insertIndex = mergedData.findIndex(
                        (c) => c.time > preservedPlaceholder.time
                      );
                      if (insertIndex >= 0) {
                  ...
                  console.log(
                    `[BitcoinChart] Merging data: ${currentData.length} existing + ${data.length} new = ${mergedData.length} total`
                  );
  ...
  return (
    <Box style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      >
        ...
        >
          <IconCircleFilled size={8} color={streamStatus.connected ? '#00ff88' : '#ff4976'} />
