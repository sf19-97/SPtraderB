...
const AdaptiveChart: React.FC<AdaptiveChartProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      ...
      console.log(
        '[AdaptiveChart] Reloading data for new symbol:',
        symbol,
        'with timeframe:',
  ...
  useEffect(() => {
    console.log(
      '[AdaptiveChart] Current state - Symbol:',
      symbol,
      'Actual Timeframe:',
      currentTimeframeRef.current
    );
  ...
  const getSymbolDateRange = async (): Promise<{ from: number; to: number }> => {
    ...
    try {
      ...
      const metadata = await invoke<SymbolMetadata>('get_symbol_metadata', {
        symbol: currentSymbol,
      });
      const fetchTime = performance.now() - startTime;
      console.log(
        `[AdaptiveChart] Symbol metadata fetched in ${fetchTime.toFixed(0)}ms:`,
        metadata
      );
  ...
  const loadHistoricalData = async (timeframe: string, from: number, to: number) => {
    try {
      ...
      console.log(
        `[AdaptiveChart] Background load complete: added ${formattedHistorical.length} historical candles, ${uniqueData.length} total after deduplication`
      );
    } catch (error) {
  ...
  const checkTimeframeSwitch = (barSpacing: number) => {
    ...
    console.log(
      '[ResolutionTracker] Checking timeframe switch - Current:',
      currentTf,
      'Bar spacing:',
      barSpacing
    );
  ...
  const loadData = async (timeframe: string) => {
    console.log(
      '[AdaptiveChart] loadData called for symbol:',
      symbolRef.current,
      'timeframe:',
      timeframe
    );
    ...
    try {
      ...
      console.log(
        '[AdaptiveChart] Full date range:',
        new Date(dateRange.from * 1000).toISOString(),
        'to',
        new Date(dateRange.to * 1000).toISOString()
      );
      ...
      console.log(
        '[AdaptiveChart] Initial load range:',
        new Date(initialFrom * 1000).toISOString(),
        'to',
        new Date(dateRange.to * 1000).toISOString()
      );
      ...
      if (formattedData.length > 0) {
        ...
        console.log(
          `[DATA BOUNDS] ${new Date(firstCandle * 1000).toISOString()} to ${new Date(lastCandle * 1000).toISOString()}`
        );
      }
      ...
      console.log(
        `[TIMING] TOTAL LOAD TIME: ${totalTime.toFixed(0)}ms (${(totalTime / 1000).toFixed(1)}s)`
      );
    } catch (error) {
  ...
  const loadDataAndMaintainView = async (
    timeframe: string,
    visibleRange: any,
    previousBarSpacing: number,
    previousTimeframe: string
  ) => {
    console.log(
      '[AdaptiveChart] loadDataAndMaintainView called for symbol:',
      symbolRef.current,
      'timeframe:',
      timeframe
    );
    ...
    try {
        ...
        if (isShiftPressed && lockedLeftEdgeRef.current !== null) {
          ...
          const ratio =
            timeframe === previousTimeframe
              ? 1
              : timeframe === '15m' && previousTimeframe === '1h'
                ? 4
                : timeframe === '1h' && previousTimeframe === '15m'
                  ? 0.25
                  : timeframe === '1h' && previousTimeframe === '4h'
                    ? 4
                    : timeframe === '4h' && previousTimeframe === '1h'
                      ? 0.25
                      : timeframe === '4h' && previousTimeframe === '12h'
                        ? 3
                        : timeframe === '12h' && previousTimeframe === '4h'
                          ? 0.33
                          : 1;
      ...
      console.log(
        `[ResolutionTracker] Loaded ${timeframe}: ${data.length} candles (maintained view)`
      );
