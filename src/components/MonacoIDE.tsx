...
execution:
  ...
  const loadWorkspace = async () => {
    try {
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] Error: Failed to load workspace`,
      ]);
    }
  };
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && moveMode) {
        ...
        setTerminalOutput((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] Move cancelled`,
        ]);
      }
    };
  ...
  useEffect(() => {
    if (terminalScrollRef.current) {
      const scrollArea = terminalScrollRef.current.querySelector(
  ...
  const handleStatusChange = (newStatus: string | null) => {
    ...
    let updatedCode = code.replace(/(['"]status['"]\s*:\s*['"])[^'"]+(['"])/g, `$1${newStatus}$2`);
    ...
    if (updatedCode === code && code.includes('__metadata__')) {
      ...
      if (!added) {
        setTerminalOutput((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] Warning: Could not update status in metadata`,
        ]);
      } else {
        setCode(updatedCode);
      }
  ...
  const handleCreateFile = async () => {
    if (!newFileName.trim()) {
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] Error: File name is required`,
      ]);
      return;
    }
    ...
    try {
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] ✓ File created successfully`,
      ]);
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] ❌ Error creating file: ${error}`,
      ]);
    }
  };
  ...
  const handleRun = async () => {
    ...
    if (!selectedFile.endsWith('.py')) {
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] Error: Can only run Python files`,
      ]);
      return;
    }
    ...
    try {
      ...
      if (dataSourceMode === 'live') {
        ...
        setTerminalOutput((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] 📊 Running in Live mode: ${liveDataParams.symbol} ${liveDataParams.timeframe}`,
        ]);
      } else {
        ...
        setTerminalOutput((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] 📄 Running in Parquet mode: ${selectedDataset || 'default'}`,
        ]);
      }
  ...
  const handleDelete = async () => {
    ...
    setTerminalOutput((prev) => [
      ...prev,
      `[${new Date().toLocaleTimeString()}] 🔄 Deleting ${fileToDelete.name}...`,
    ]);
    ...
    try {
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] ✓ Deleted ${fileToDelete.name}`,
      ]);
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] ❌ Error deleting: ${error}`,
      ]);
    }
  };
  ...
  const handleMoveFile = async (sourcePath: string, targetFolderPath: string) => {
    try {
      ...
      if (!fileName) {
        setTerminalOutput((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] ❌ Error: Invalid source path`,
        ]);
        return;
      }
      ...
      if (sourcePath === newPath) {
        setTerminalOutput((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] File is already in this location`,
        ]);
        return;
      }
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] Moving ${fileName} to ${targetFolderPath}...`,
      ]);
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] ✓ Moved successfully to ${result}`,
      ]);
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] ❌ Error moving file: ${error}`,
      ]);
      setMoveMode(false);
      setFileToMove(null);
    }
  ...
  const handleRename = async () => {
    ...
    if (isFolder) {
      if (!renameFileName.match(/^[a-zA-Z0-9_-]+$/)) {
        setTerminalOutput((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] ❌ Error: Invalid folder name. Use only letters, numbers, underscore, and dash.`,
        ]);
        return;
      }
    } else {
      if (!renameFileName.match(/^[a-zA-Z0-9_-]+\.(py|yaml|yml)$/)) {
        setTerminalOutput((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] ❌ Error: Invalid file name. Use only letters, numbers, underscore, and dash.`,
        ]);
        return;
      }
    }
    ...
    try {
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] ✓ Renamed to ${renameFileName}`,
      ]);
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] ❌ Error renaming: ${error}`,
      ]);
    }
  };
  ...
  const handleExportData = async () => {
    if (!exportStartDate || !exportEndDate) {
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] ❌ Error: Please select start and end dates`,
      ]);
      return;
    }
    ...
    try {
      ...
      const startDate =
        exportStartDate instanceof Date ? exportStartDate : new Date(exportStartDate);
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] 🚀 Exporting ${exportSymbol} ${exportTimeframe} data...`,
      ]);
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] ✓ Data exported to: ${parquetFilename}`,
      ]);
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] 🎉 Ready to use! Run your component to load this data.`,
      ]);
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] ❌ Export failed: ${error}`,
      ]);
    } finally {
  ...
  const loadLiveData = async () => {
    try {
      ...
      setTerminalOutput((prev) => [
        ...prev,
        `[${new Date().toLocaleTimeString()}] 🔄 Loading ${symbol} ${timeframe} data...`,
      ]);
      ...
      if (data && data.length > 0) {
        ...
        setTerminalOutput((prev) => [
          ...prev,
          `[${new Date().toLocaleTimeString()}] ✓ Loaded ${data.length} candles`,
        ]);
      } else {
        setTerminalOutput((prev) => [
          ...prev,
  ...
  const renderFileTree = (nodes: FileNode[], depth: number = 0) => {
    ...
    return nodes.map((node) => {
      ...
      const isAllowed = allowedPaths[type].some(
        (allowed) => node.path.startsWith(allowed) || allowed.startsWith(node.path)
      );
      ...
      if (node.type === 'folder') {
        ...
        const isValidMoveTarget =
          moveMode && canAcceptFiles && fileToMove && fileToMove.type === 'file';
          ...
          <div key={node.path}>
            <UnstyledButton
              ...
              style={{
                ...
                transition: 'all 0.2s',
                '&:hover': {
                  backgroundColor: isValidMoveTarget
                    ? 'rgba(0, 201, 189, 0.2)'
            ...
            {isExpanded && node.children && <div>{renderFileTree(node.children, depth + 1)}</div>}
          </div>
        );
      } else {
        const icon = node.name.endsWith('.py') ? (
          ...
          <UnstyledButton
            ...
            onClick={async () => {
              ...
              try {
                ...
                setTerminalOutput((prev) => [
                  ...prev,
                  `[${new Date().toLocaleTimeString()}] Loaded ${node.path}`,
                ]);
                ...
                setTerminalOutput((prev) => [
                  ...prev,
                  `[${new Date().toLocaleTimeString()}] ❌ Error loading ${node.path}: ${error}`,
                ]);
              } finally {
                setIsLoading(false);
              }
            ...
            style={{
              ...
              backgroundColor: isFileBeingMoved
                ? 'rgba(255, 153, 0, 0.1)'
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Group gap="sm">
            <ActionIcon onClick={() => navigate('/build')} variant="subtle" c="gray" size="sm">
              <IconChevronLeft size={16} />
            </ActionIcon>
            <Group gap="xs">
              <IconFile size={16} style={{ color: '#888' }} />
              <Text size="sm" c="#cccccc">
                {selectedFile || 'untitled.py'}
              </Text>
              <Text size="xs" c="#888">
                • Python
              </Text>
      ...
      {/* Preview Panel */}
      {
        ...
        >
          ...
          <Box style={{ flex: 1, padding: '16px' }}>
                ...
                <Select
                  ...
                  onChange={async (value) => {
                    ...
                    if (value) {
                      setTerminalOutput((prev) => [
                        ...prev,
                        `[${new Date().toLocaleTimeString()}] Selected dataset: ${value}`,
                      ]);
                      ...
                      try {
                        ...
                        setChartData((prevData: any) => {
                          ...
                          if (prevData?.indicators || prevData?.signals) {
                            console.log(
                              '[Chart] Preserving component output data, ignoring dataset load'
                            );
                            return prevData;
                          }
                          return data;
                        ...
                        setTerminalOutput((prev) => [
                          ...prev,
                          `[${new Date().toLocaleTimeString()}] ✓ Chart data loaded (${data.time.length} candles)`,
                        ]);
                      } catch (error) {
                        setTerminalOutput((prev) => [
                          ...prev,
            ...
            <Stack gap="xs" mt="md">
              <Text size="xs" fw={500} c="#888" mb={4}>
                COMPONENT OUTPUT
              </Text>
              <Group justify="space-between">
                ...
                <Text size="sm" c="#cccccc" ff="monospace">
                  {componentOutput.lastValue}
                </Text>
              </Group>
              <Group justify="space-between">
                <Text size="sm" c="#888">
                ...
                <Text
                  size="sm"
                  c={
                    componentOutput.signal === 'BUY'
                      ? '#4ec9b0'
                      : componentOutput.signal === 'SELL'
                        ? '#f48771'
                        : '#666'
                  }
                  ff="monospace"
              ...
              <Group justify="space-between">
                ...
                <Text size="sm" c="#666" ff="monospace">
                  {componentOutput.execution}
                </Text>
              </Group>
              <Group justify="space-between">
                <Text size="sm" c="#888">
            ...
            <Text size="xs" c="#555" mt="xl">
              Export test data using the database icon above, then select a dataset to preview your
            ...
            <>
              ...
              {isCustomCategory && (
                <TextInput
                  ...
                  placeholder={
                    type === 'indicator'
                      ? 'e.g., orderflow, market_profile'
                      : type === 'signal'
                        ? 'e.g., pattern_recognition, volume_analysis'
                        : 'e.g., iceberg, twap, vwap'
                  }
                  value={customCategoryName}
                  onChange={(e) =>
                    setCustomCategoryName(e.currentTarget.value.toLowerCase().replace(/\s+/g, '_'))
                  }
              ...
              <UnstyledButton
                ...
                onClick={(e) => {
                  ...
                  if (file) {
                    ...
                    setTerminalOutput((prev) => [
                      ...prev,
                      `[${new Date().toLocaleTimeString()}] Move mode: Click a folder to move ${file.name} (ESC to cancel)`,
                    ]);
                  }
                }}
                onMouseEnter={(e) => {
      ...
      >
        <Stack gap="md">
          <Text>
            Are you sure you want to delete {fileToDelete?.type === 'folder' ? 'folder' : 'file'}{' '}
