...
const MarketDataChart: React.FC<MarketDataChartProps> = ({ 
  ...
  const updateCountdown = useCallback(() => {
    ...
    if (totalSeconds <= 5 && totalSeconds >= -5) {
      console.log(`[MarketChart] Countdown: ${totalSeconds}s, triggered: ${hasTriggeredNewCandleRef.current}, time: ${date.toISOString()}`);
    }
      ...
      setTimeout(() => {
        ...
        if (seriesRef.current) {
          ...
          if (currentData.length > 0) {
            ...
            console.log('[MarketChart] Placeholder candle created precisely at', new Date(newCandleTime * 1000).toLocaleTimeString());
    ...
    if (totalSeconds <= 0 && totalSeconds > -1 && !hasTriggeredNewCandleRef.current) {
      ...
      console.log(`[MarketChart] PLACEHOLDER TRIGGER at ${exactTime.toISOString()} (totalSeconds: ${totalSeconds}, ms: ${exactTime.getMilliseconds()})`);
      ...
      if (seriesRef.current) {
        ...
        if (currentData.length > 0) {
          ...
          const newData = [...currentData, placeholderCandle];
          seriesRef.current.setData(newData as any);
          console.log('[MarketChart] Placeholder candle created at', new Date(newCandleTime * 1000).toLocaleTimeString());
  ...
  const fetchChartData = async (sym: string, tf: string, from?: number, to?: number): Promise<{ data: ChartData[], metadata: SymbolMetadata | null }> => {
    try {
      ...
      console.log(`[MarketChart] Fetching ${tf} data from ${new Date(from * 1000).toISOString()} to ${new Date(to * 1000).toISOString()}`);
      ...
      if (response.data && response.data.length > 0) {
        const chartData = response.data.map((candle: any, index: number) => {
          ...
          if (index < 3) {
            ...
            console.log(`[MarketChart] Unix timestamp:`, Math.floor(new Date(candle.time).getTime() / 1000));
          }
        ...
        if (chartData.length > 0) {
          ...
          console.log('[MarketChart] First candle:', firstDate.toISOString(), '(Local:', firstDate.toLocaleString(), ')');
          console.log('[MarketChart] Last candle:', lastDate.toISOString(), '(Local:', lastDate.toLocaleString(), ')');
  ...
  useEffect(() => {
    ...
    const unsubscribe = chart.subscribeCrosshairMove(param => {
      ...
      const timestamp = typeof param.time === 'string' ? parseInt(param.time) * 1000 : (param.time as number) * 1000;
  ...
  useEffect(() => {
    ...
    const loadInitialData = async () => {
      if (!mounted || initialLoadDoneRef.current || !seriesRef.current || !chartRef.current || !symbol) return;
      ...
      try {
        ...
        const { data } = await fetchChartData(symbol || 'EURUSD', currentTimeframeRef.current, from, to);
        if (data.length > 0) {
          seriesRef.current.setData(data as any);
          ...
          unlistenCandle = await listen<{symbol: string, timeframe: string, timestamp: string}>(eventName, (event) => {
            ...
            if (event.payload.timeframe === currentTimeframeRef.current) {
              ...
              fetchChartData(symbolRef.current!, currentTimeframeRef.current, from, to)
                ...
                .catch(error => console.error('[MarketChart] Failed to reload after update:', error));
            }
          });
        };
  ...
  useEffect(() => {
    ...
    console.log(`[MarketChart] Syncing periodic refresh - current: :${currentSecond}, next: :${nextTarget % 60}, delay: ${delaySeconds}s`);
        ...
        .then(({ data }) => {
            if (data.length > 0 && seriesRef.current) {
              ...
              if (placeholderTimeRef.current && data.length > 0) {
                const realCandle = data.find((c: ChartData) => c.time === placeholderTimeRef.current);
                if (realCandle) {
              ...
              if (currentData.length > 0 && data.length > 0) {
                ...
                const existingIndex = currentData.findIndex(c => (c.time as number) >= firstNewTime);
                ...
                if (existingIndex >= 0) {
                  ...
                  if (placeholderTime && placeholderTime > 0) {
                    ...
                    const placeholderInCurrent = currentData.find((c: any) => c.time === placeholderTime);
                    const placeholderInNew = data.find((c: ChartData) => c.time === placeholderTime);
                    
                    if (placeholderInCurrent && !placeholderInNew) {
                      // Preserve the placeholder
                      preservedPlaceholder = placeholderInCurrent;
                      console.log('[MarketChart] Preserving placeholder candle at', new Date(placeholderTime * 1000).toLocaleTimeString());
                    }
                  }
                  ...
                  if (preservedPlaceholder) {
                    ...
                    const insertIndex = mergedData.findIndex((c: any) => c.time > (preservedPlaceholder as any).time);
                    if (insertIndex >= 0) {
                ...
                console.log(`[MarketChart] Merging data: ${currentData.length} existing + ${data.length} new = ${mergedData.length} total`);
