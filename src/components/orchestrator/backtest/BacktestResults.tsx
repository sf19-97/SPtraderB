import { Paper, Grid, Group, Text, ThemeIcon, Stack, Progress, Tabs, ScrollArea, Badge, Button, LoadingOverlay } from '@mantine/core';
import { 
  IconTrendingUp, 
  IconTrendingDown, 
  IconChartLine, 
...
export function BacktestResults() {
  ...
  const startCapital = backtestResults ? (typeof backtestResults.startCapital === 'string' 
    ? parseFloat(backtestResults.startCapital) 
    : backtestResults.startCapital) : 0;
  const endCapital = backtestResults ? (typeof backtestResults.endCapital === 'string' 
    ? parseFloat(backtestResults.endCapital) 
    : backtestResults.endCapital) : 0;
  const totalPnl = backtestResults ? (typeof backtestResults.totalPnl === 'string' 
    ? parseFloat(backtestResults.totalPnl) 
    : backtestResults.totalPnl) : 0;
  const maxDrawdown = backtestResults ? (typeof backtestResults.maxDrawdown === 'string' 
    ? parseFloat(backtestResults.maxDrawdown) 
    : backtestResults.maxDrawdown) : 0;

  const totalReturn = startCapital > 0 ? ((endCapital - startCapital) / startCapital) * 100 : 0;
  const winRate = backtestResults && backtestResults.totalTrades > 0 
  ...
  const chartTrades = useMemo(() => {
    ...
    if ('completed_trades' in backtestResults && Array.isArray(backtestResults.completed_trades)) {
      ...
      return backtestResults.completed_trades.map((trade: any) => ({
        ...
        entryPrice: typeof trade.entry_price === 'string' ? parseFloat(trade.entry_price) : trade.entry_price,
        exitPrice: typeof trade.exit_price === 'string' ? parseFloat(trade.exit_price) : trade.exit_price,
        ...
        pnlPercent: typeof trade.pnl_percent === 'string' ? parseFloat(trade.pnl_percent) : trade.pnl_percent,
    ...
    <Tabs value={activeResultsTab} onChange={setActiveResultsTab}>
      ...
      <Tabs.Panel value="overview" pt="md">
        ...
        <Grid.Col span={{ base: 12, sm: 6, md: 4 }}>
          <StatCard
            ...
            icon={totalReturn >= 0 ? <IconTrendingUp size={28} /> : <IconTrendingDown size={28} />}
      ...
      <Paper p="md" withBorder>
        <Stack gap="sm">
          ...
          <Group justify="space-between">
            <Text size="sm" c="green">Winning: {backtestResults?.winningTrades || 0}</Text>
            <Text size="sm" c="red">Losing: {backtestResults?.losingTrades || 0}</Text>
          </Group>
          <Progress
            size="xl"
            value={backtestResults?.totalTrades > 0 ? (backtestResults.winningTrades / backtestResults.totalTrades) * 100 : 0}
            color="green"
          />
        </Stack>
      </Paper>
          ...
          <Stack gap="sm">
            ...
            <Grid>
              <Grid.Col span={6}>
                ...
                <Text fw={600}>${(typeof backtestResults.finalPortfolio.cash === 'string' ? parseFloat(backtestResults.finalPortfolio.cash) : backtestResults.finalPortfolio.cash).toFixed(2)}</Text>
              </Grid.Col>
              <Grid.Col span={6}>
                <Text size="sm" c="dimmed">Portfolio Value</Text>
                <Text fw={600}>${(typeof backtestResults.finalPortfolio.total_value === 'string' ? parseFloat(backtestResults.finalPortfolio.total_value) : backtestResults.finalPortfolio.total_value).toFixed(2)}</Text>
              </Grid.Col>
              <Grid.Col span={6}>
                <Text size="sm" c="dimmed">Open Positions</Text>
                <Text fw={600}>{Object.keys(backtestResults.finalPortfolio.positions || {}).length}</Text>
              </Grid.Col>
              <Grid.Col span={6}>
      ...
      <Tabs.Panel value="logs" pt="md">
        <Stack h="500px" gap="xs">
          <Group justify="space-between">
            <Group>
              <Text fw={600} size="sm">System Logs</Text>
          ...
          <ScrollArea h="100%" offsetScrollbars>
            <Stack gap={4}>
              ...
              {logs.length === 0 && (
                <Text size="xs" c="dimmed" ta="center">No logs yet...</Text>
              )}
            </Stack>
          </ScrollArea>
        </Stack>
