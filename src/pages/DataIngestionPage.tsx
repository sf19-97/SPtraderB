...
export const DataIngestionPage = () => {
  ...
  const addTestLogs = () => {
    ...
    setTimeout(
      () => addLog('python', 'Downloading EURUSD: 0%|          | 0/172 [00:00<?, ?it/s]'),
      500
    );
    setTimeout(() => addLog('db', 'Executing bulk insert: 3,847 rows'), 1000);
    setTimeout(
      () => addLog('python', 'Downloading EURUSD: 10%|█         | 17/172 [00:30<05:26, 2.10s/it]'),
      1500
    );
    ...
    setTimeout(
      () => addLog('python', 'Downloading EURUSD: 20%|██        | 34/172 [01:00<04:00, 1.74s/it]'),
      5000
    );
  ...
  useEffect(() => {
    ...
    const unlistenRefresh = listen<{ symbol: string; progress: number; stage: string }>(
      'candle-refresh-progress',
      (event) => {
    ...
    const unlistenBackendLog = listen<{ timestamp: string; level: string; message: string }>(
      'backend-log',
      (event) => {
  ...
  const cancelIngestion = async () => {
    ...
    try {
      const cancelled = await invoke<boolean>('cancel_ingestion', {
        symbol: currentIngestionSymbol,
      });
      if (cancelled) {
  ...
  const startIngestion = async () => {
    ...
    addLog(
      'info',
      `Starting ingestion for ${selectedPair} from ${startDate!.toISOString().split('T')[0]} to ${endDate!.toISOString().split('T')[0]}`
    );
                ...
                {availableData.map((data) => (
                  <Table.Tr key={data.symbol}>
                    ...
                    <Table.Td>
                      <Text size="sm">
                        {(
                          data.candle_count_5m +
                          data.candle_count_15m +
                          data.candle_count_1h +
                          data.candle_count_4h +
                          data.candle_count_12h
                        ).toLocaleString()}
                      </Text>
                    </Table.Td>
                    <Table.Td>
                      ...
                      {refreshProgress[data.symbol] ? (
                        <Stack gap={4}>
                          ...
                          <Text size="xs" c="dimmed">
                            {refreshProgress[data.symbol].stage}
                          </Text>
                        </Stack>
                      ) : (
                        <Group gap="xs">
        ...
        >
          <Stack>
            ...
            <Group justify="flex-end" mt="xl">
              ...
              <Button color="blue" onClick={confirmModal.onConfirm}>
                Confirm
              </Button>
            </Group>
          </Stack>
