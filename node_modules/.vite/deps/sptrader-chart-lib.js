import {
  ...
  Select,
  Text
} from "./chunk-R7K2E356.js";
import "./chunk-NXESFFTV.js";
import {
  require_jsx_runtime
} from "./chunk-6PXSGDAH.js";
import "./chunk-ULKCAQZZ.js";
import {
  require_react
...
// node_modules/sptrader-chart-lib/dist/index.mjs
...
// node_modules/sptrader-chart-lib/node_modules/lightweight-charts/dist/lightweight-charts.development.mjs
...
// node_modules/sptrader-chart-lib/node_modules/zustand/esm/vanilla.mjs
...
// node_modules/sptrader-chart-lib/node_modules/zustand/esm/index.mjs
...
// node_modules/sptrader-chart-lib/node_modules/zustand/esm/middleware.mjs
...
var Logger = class _Logger {
  ...
  log(level, message, ...args) {
    ...
    const logMethod = level === 0 ? console.error : level === 1 ? console.warn : console.log;
    if (this.config.enableColors && typeof window === "undefined") {
      const color2 = this.colors[level];
      logMethod(`${color2}${formattedMessage}${this.colors.reset}`, ...args);
    } else {
...
if (typeof window !== "undefined") {
  const logLevel = window.__CHART_LOG_LEVEL__ ? LogLevel[window.__CHART_LOG_LEVEL__] || 2 : true ? 2 : 1;
  logger.configure({ level: logLevel });
} else {
  const logLevel = process.env.LOG_LEVEL ? LogLevel[process.env.LOG_LEVEL] || 2 : 2;
  logger.configure({ level: logLevel });
}
var createLogger = (prefix) => logger.child(prefix);
var CACHE_TTL = 10 * 60 * 1e3;
var logger2 = createLogger("useChartStore");
var useChartStore = create()(
  devtools(
    (set, get) => ({
      // Initial state
...
var alignToCandleBoundary = (timestamp, interval) => {
  if (interval === 14400) {
    ...
    const alignedHours = Math.floor(hours2 / 4) * 4;
    date.setUTCHours(alignedHours, 0, 0, 0);
    return Math.floor(date.getTime() / 1e3);
  } else if (interval === 43200) {
    const date = new Date(timestamp * 1e3);
    const hours2 = date.getUTCHours();
    const alignedHours = hours2 < 12 ? 0 : 12;
    date.setUTCHours(alignedHours, 0, 0, 0);
    return Math.floor(date.getTime() / 1e3);
  }
...
var ChartDataCoordinator = class {
  ...
  async calculateDefaultRange(symbol, timeframe) {
    ...
    const dataEndTime = (metadata == null ? void 0 : metadata.data_to) || Math.floor(Date.now() / 1e3);
    const dataStartTime = (metadata == null ? void 0 : metadata.data_from) || dataEndTime - 365 * 24 * 60 * 60;
    const to = dataEndTime;
    let from;
    switch (timeframe) {
      case "1m":
      case "5m":
    ...
    if (from < dataStartTime) {
      from = dataStartTime;
    }
    this.logger.debug(
      `Calculated default range for ${symbol} ${timeframe}: ${new Date(from * 1e3).toISOString()} to ${new Date(to * 1e3).toISOString()}`
    );
    return { from, to };
  }
  /**
  ...
  async fetchChartData(symbol, timeframe, options) {
    ...
    let range;
    if (options == null ? void 0 : options.range) {
      range = options.range;
    } else {
  ...
  renderer() {
    return {
      draw: (target) => {
        ...
        const { countdown, color: color2, position, offset } = this._primitive.getData();
        ctx.save();
...
var CountdownTimerPrimitive = class {
  ...
  updateTimeframe(timeframe) {
    ...
    (_a = this._requestUpdate) == null ? void 0 : _a.call(this);
  }
  attached(param) {
  ...
  _calculateSecondsToNextCandle() {
    ...
    switch (this._timeframe) {
      case "5m":
        return (5 - minutes2 % 5) * 60 - seconds2;
      case "15m":
        return (15 - minutes2 % 15) * 60 - seconds2;
      case "1h":
        return (60 - minutes2) * 60 - seconds2;
      case "4h":
        return (4 - hours2 % 4) * 3600 + (60 - minutes2) * 60 - seconds2;
      case "12h":
        return (12 - hours2 % 12) * 3600 + (60 - minutes2) * 60 - seconds2;
      default:
        return (60 - minutes2) * 60 - seconds2;
    }
  }
  _formatCountdown(totalSeconds) {
    ...
    const minutes2 = Math.floor(displaySeconds / 60);
    const seconds2 = displaySeconds % 60;
    return `${minutes2.toString().padStart(2, "0")}:${seconds2.toString().padStart(2, "0")}`;
  }
  _getCountdownColor(seconds2) {
    if (seconds2 <= 10) {
      return "#ffae00";
    } else if (seconds2 <= 30) {
      return "#ccc";
  ...
  _updateCountdown() {
    ...
    if (totalSeconds <= 0 && !this._hasTriggered) {
      ...
      (_a = this._onNewCandleBoundary) == null ? void 0 : _a.call(this, candleTime);
      logger5.debug("New candle boundary:", candleTime);
    }
    if (totalSeconds > 5) {
      this._hasTriggered = false;
    }
    (_b = this._requestUpdate) == null ? void 0 : _b.call(this);
  }
  getData() {
    return {
...
var TimeframeWatermarkPrimitive = class {
  constructor(options) {
    ...
    this._paneViews = [new WatermarkPaneView2(this)];
  }
  updateTimeframe(timeframe) {
    var _a;
    this._timeframe = timeframe;
...
function usePlaceholderCandle(series, options) {
  ...
  const hasTriggeredRef = (0, import_react2.useRef)(false);
  const placeholderTimeRef = (0, import_react2.useRef)(null);
  const resetTimeoutRef = (0, import_react2.useRef)(null);
  const createPlaceholder = (0, import_react2.useCallback)((candleTime) => {
    ...
    onPlaceholderCreated == null ? void 0 : onPlaceholderCreated(candleTime);
  ...
  const updateWithRealData = (0, import_react2.useCallback)((data) => {
  ...
  const hasPlaceholder = (0, import_react2.useCallback)(() => {
    return placeholderTimeRef.current !== null;
  }, []);
  const getPlaceholderTime = (0, import_react2.useCallback)(() => {
    return placeholderTimeRef.current;
  }, []);
  const resetTrigger = (0, import_react2.useCallback)(() => {
    hasTriggeredRef.current = false;
    placeholderTimeRef.current = null;
    if (resetTimeoutRef.current) {
      clearTimeout(resetTimeoutRef.current);
...
function createPlaceholderCandle(lastCandle, timeframe, currentTime) {
  ...
  const price = (lastCandle == null ? void 0 : lastCandle.close) ?? (lastCandle == null ? void 0 : lastCandle.open) ?? 0;
  return {
    time: candleTime,
    open: price,
    high: price,
...
var ErrorBoundary = class extends import_react2.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (hasError && prevProps.resetKeys !== resetKeys) {
      if (resetKeys == null ? void 0 : resetKeys.some((key, idx) => {
        var _a;
        return key !== ((_a = prevProps.resetKeys) == null ? void 0 : _a[idx]);
      })) {
        this.resetErrorBoundary();
      }
    }
    ...
    if (hasError && error) {
      ...
      return (0, import_jsx_runtime.jsxs)(
        "div",
        {
          ...
          children: [
            (0, import_jsx_runtime.jsx)("h2", { style: { color: "#c92a2a", marginTop: 0 }, children: "Something went wrong" }),
            (0, import_jsx_runtime.jsxs)("details", { style: { whiteSpace: "pre-wrap", marginBottom: "20px" }, children: [
              (0, import_jsx_runtime.jsx)("summary", { style: { cursor: "pointer", color: "#666" }, children: "Error details" }),
              (0, import_jsx_runtime.jsx)("p", { style: { color: "#666", marginTop: "10px" }, children: error.toString() }),
              errorInfo && (0, import_jsx_runtime.jsx)("pre", { style: { color: "#666", fontSize: "12px", overflow: "auto" }, children: errorInfo.componentStack })
            ] }),
...
var ChartErrorBoundary = ({
  ...
  return (0, import_jsx_runtime.jsx)(
    ...
    {
      onError,
      fallback: (error, _errorInfo, retry) => (0, import_jsx_runtime.jsxs)(
        "div",
        {
          style: {
            display: "flex",
          ...
          children: [
            ...
            (0, import_jsx_runtime.jsxs)("div", { style: { display: "flex", gap: "10px" }, children: [
              (0, import_jsx_runtime.jsx)(
                "button",
                {
                  onClick: retry,
...
var LoadingState = ({
  ...
  const content = (0, import_jsx_runtime.jsxs)(
    "div",
    {
      style: {
        display: "flex",
      ...
      children: [
        showSpinner && (0, import_jsx_runtime.jsx)(
          "div",
          {
            style: {
              width: "40px",
        ...
        subMessage && (0, import_jsx_runtime.jsx)(
          "div",
          {
            style: {
              fontSize: "14px",
      ...
      children: [
        ...
        (0, import_jsx_runtime.jsx)(
          "div",
          {
            ...
            children: (0, import_jsx_runtime.jsx)("div", { style: { display: "flex", justifyContent: "space-around", height: "100%", alignItems: "flex-end" }, children: [...Array(8)].map((_, i) => (0, import_jsx_runtime.jsx)(
              "div",
              {
                style: {
                  width: "8px",
  ...
  if (!symbol) {
    return (0, import_jsx_runtime.jsx)(
      EmptyState,
      {
        ...
        icon: (0, import_jsx_runtime.jsx)("div", { style: { fontSize: "48px", marginBottom: "20px", color: "#dee2e6" }, children: "🔍" }),
...
var MarketDataChart = ({
  ...
  const seriesRef = (0, import_react2.useRef)(null);
  const countdownTimerRef = (0, import_react2.useRef)(null);
  const watermarkRef = (0, import_react2.useRef)(null);
  const [currentTimeframe, setCurrentTimeframe] = (0, import_react2.useState)(timeframe || "1h");
  const currentTimeframeRef = (0, import_react2.useRef)(timeframe || "1h");
  const symbolRef = (0, import_react2.useRef)(symbol || "EURUSD");
  const [isLoading, setIsLoading] = (0, import_react2.useState)(false);
  const [_error, setError] = (0, import_react2.useState)(null);
  const isTransitioningRef = (0, import_react2.useRef)(false);
  const {
  ...
  const lastTransitionRef = (0, import_react2.useRef)(0);
  const TRANSITION_COOLDOWN = 700;
  const [isShiftPressed, setIsShiftPressed] = (0, import_react2.useState)(false);
  const lockedLeftEdgeRef = (0, import_react2.useRef)(null);
  const checkIntervalRef = (0, import_react2.useRef)(null);
  const initialLoadDoneRef = (0, import_react2.useRef)(false);
  ...
  const switchTimeframe = async (newTimeframe) => {
    ...
    (_a = watermarkRef.current) == null ? void 0 : _a.updateTimeframe(newTimeframe);
    (_b = countdownTimerRef.current) == null ? void 0 : _b.updateTimeframe(newTimeframe);
    try {
      logger8.debug(`Starting fetch for ${symbolRef.current} ${newTimeframe}`);
      ...
      if (data.length > 0 && seriesRef.current && chartRef.current) {
        ...
        logger8.debug(`Updated currentTimeframeRef.current to: ${currentTimeframeRef.current}`);
        if (visibleRange && chartRef.current) {
          logger8.debug(`Restoring visible range: ${new Date(visibleRange.from * 1e3).toISOString()} to ${new Date(visibleRange.to * 1e3).toISOString()}`);
  ...
  (0, import_react2.useEffect)(() => {
      ...
      timeScale: {
        ...
        tickMarkFormatter: (time, tickMarkType) => {
          ...
          if (tickMarkType === 0) {
            ...
            const ampm = hours2 >= 12 ? "PM" : "AM";
            const displayHours = hours2 % 12 || 12;
            return `${displayHours}:${minutes2} ${ampm}`;
            ...
            const ampm = hours2 >= 12 ? "PM" : "AM";
            const displayHours = hours2 % 12 || 12;
            return `${displayHours}:${minutes2}:${seconds2} ${ampm}`;
          }
    ...
    const checkTimeframeSwitch = (barSpacing) => {
      ...
      if (currentTf === "12h" && barSpacing < 3) {
        ...
        (_a = chartRef.current) == null ? void 0 : _a.timeScale().applyOptions({
          barSpacing: 3
        });
        return;
      }
    ...
    const handleKeyDown = (e) => {
      ...
      if (e.key === "Shift" && !isShiftPressed) {
        ...
        const timeScale = (_a = chartRef.current) == null ? void 0 : _a.timeScale();
        if (timeScale) {
          const visibleRange = timeScale.getVisibleRange();
          if (visibleRange) {
  ...
  (0, import_react2.useEffect)(() => {
    ...
    const loadInitialData = async () => {
      ...
      try {
        const metadata = await chartDataCoordinator.getSymbolMetadata(symbol || "EURUSD");
        const dataEndTime = (metadata == null ? void 0 : metadata.data_to) || Math.floor(Date.now() / 1e3);
        const dataStartTime = metadata == null ? void 0 : metadata.data_from;
        const to = dataEndTime;
        let from;
        ...
        if (dataStartTime && from < dataStartTime) {
          from = dataStartTime;
        }
        ...
        if (data.length > 0) {
          ...
          if (chartRef.current && data.length > 0) {
            ...
            const from2 = lastDataTime - daysToShow * 24 * 60 * 60;
            chartRef.current.timeScale().setVisibleRange({
              from: from2,
              to: lastDataTime
            });
  ...
  if (isLoading && !initialLoadDoneRef.current) {
    return (0, import_jsx_runtime.jsx)(ChartErrorBoundary, { children: (0, import_jsx_runtime.jsx)(Box, { style: { position: "relative", width: "100%", height: "100%" }, children: (0, import_jsx_runtime.jsx)(ChartSkeleton, { height: "100%" }) }) });
  }
  if (!symbol) {
    return (0, import_jsx_runtime.jsx)(ChartErrorBoundary, { children: (0, import_jsx_runtime.jsx)(Box, { style: { position: "relative", width: "100%", height: "100%" }, children: (0, import_jsx_runtime.jsx)(ChartEmptyState, {}) }) });
  }
  return (0, import_jsx_runtime.jsx)(ChartErrorBoundary, { onError: (err) => setError(err), children: (0, import_jsx_runtime.jsx)(Box, { style: { position: "relative", width: "100%", height: "100%" }, children: (0, import_jsx_runtime.jsxs)(
    "div",
    {
      ...
      children: [
        isLoading && (0, import_jsx_runtime.jsx)(
          "div",
          {
            style: {
              position: "absolute",
        ...
        (0, import_jsx_runtime.jsxs)(
          "div",
          {
            ...
            children: [
              currentTimeframe,
              isShiftPressed && (0, import_jsx_runtime.jsx)("span", { style: { marginLeft: "10px", color: "#ff9900" }, children: "[LOCK LEFT]" })
            ]
          }
        )
      ]
...
var logger9 = createLogger("ReconnectionManager");
var ReconnectionManager = class {
  ...
  setStatus(status) {
    if (this.currentStatus !== status) {
      ...
      logger9.info(`Status changed: ${status}`);
      this.config.onStatusChange(status);
    }
  }
  async connect() {
    if (this.isDestroyed) {
      logger9.warn("Cannot connect: ReconnectionManager is destroyed");
      return;
    }
  ...
  handleConnectionSuccess() {
    logger9.info("Connection successful");
    this.retryCount = 0;
    this.currentDelay = this.config.initialDelay;
  ...
  handleConnectionError(error) {
    logger9.error("Connection failed:", error);
  ...
  handleMaxRetriesReached() {
    logger9.error(`Max retries (${this.config.maxRetries}) reached`);
  ...
  scheduleRetry() {
    ...
    logger9.info(`Scheduling retry ${this.retryCount}/${this.config.maxRetries} in ${this.currentDelay}ms`);
  ...
  disconnect(reason) {
    logger9.info("Disconnecting", reason ? `Reason: ${reason}` : "");
    this.clearRetryTimer();
  ...
  destroy() {
    logger9.info("Destroying ReconnectionManager");
...
var WebSocketReconnectionManager = class extends ReconnectionManager {
  ...
  constructor(url, protocols, config) {
    ...
    if (config == null ? void 0 : config.enableHeartbeat) {
  ...
  async createWebSocket(onMessage) {
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.onopen = () => {
          logger9.info("WebSocket connected");
          this.startHeartbeat();
          resolve();
        };
        this.ws.onerror = (event) => {
          logger9.error("WebSocket error:", event);
          reject(new Error("WebSocket connection failed"));
        };
        this.ws.onclose = (event) => {
          logger9.info("WebSocket closed:", { code: event.code, reason: event.reason });
          this.stopHeartbeat();
  ...
  sendPing() {
    ...
    if (((_a = this.ws) == null ? void 0 : _a.readyState) === WebSocket.OPEN) {
      ...
      this.heartbeatTimeout = setTimeout(() => {
        var _a2;
        logger9.warn("Heartbeat timeout - no pong received");
  ...
  handlePong() {
    ...
    if (this.lastPingTime) {
      ...
      logger9.trace(`Heartbeat latency: ${latency}ms`);
    }
  }
  send(data) {
    var _a;
    if (((_a = this.ws) == null ? void 0 : _a.readyState) === WebSocket.OPEN) {
      ...
      logger9.warn("Cannot send: WebSocket is not open");
    }
  }
  close(code, reason) {
    var _a;
    this.stopHeartbeat();
    (_a = this.ws) == null ? void 0 : _a.close(code, reason);
    this.destroy();
  }
  getWebSocket() {
    return this.ws;
...
var logger10 = createLogger("useReconnection");
function useReconnection(connectFn, options = {}) {
  ...
  const [nextRetryDelay, setNextRetryDelay] = (0, import_react2.useState)(0);
  const managerRef = (0, import_react2.useRef)();
  (0, import_react2.useEffect)(() => {
    if (!enabled) return;
    const manager = new ReconnectionManager(connectFn, {
      ...
      onStatusChange: (newStatus) => {
        ...
        (_a = config.onStatusChange) == null ? void 0 : _a.call(config, newStatus);
      }
    });
  ...
  const connect = (0, import_react2.useCallback)(async () => {
    var _a;
    await ((_a = managerRef.current) == null ? void 0 : _a.connect());
  }, []);
  const disconnect = (0, import_react2.useCallback)((reason) => {
    var _a;
    (_a = managerRef.current) == null ? void 0 : _a.disconnect(reason);
  }, []);
  const resetRetries = (0, import_react2.useCallback)(() => {
    var _a;
    (_a = managerRef.current) == null ? void 0 : _a.resetRetryCount();
    setRetryCount(0);
...
function useWebSocketReconnection(options) {
  ...
  const [ws, setWs] = (0, import_react2.useState)();
  const managerRef = (0, import_react2.useRef)();
  (0, import_react2.useEffect)(() => {
    if (!enabled) return;
    const manager = new WebSocketReconnectionManager(url, protocols, {
      ...
      onConnected: () => {
        ...
        (_a2 = reconnectionConfig.onConnected) == null ? void 0 : _a2.call(reconnectionConfig);
      },
      onDisconnected: (reason) => {
        ...
        (_a2 = reconnectionConfig.onDisconnected) == null ? void 0 : _a2.call(reconnectionConfig, reason);
      }
    });
    managerRef.current = manager;
    if (autoConnect) {
  ...
  const send = (0, import_react2.useCallback)((data) => {
    var _a2;
    (_a2 = managerRef.current) == null ? void 0 : _a2.send(data);
  }, []);
  const close = (0, import_react2.useCallback)((code, reason) => {
    var _a2;
    (_a2 = managerRef.current) == null ? void 0 : _a2.close(code, reason);
  }, []);
  return {
    ...reconnectionReturn,
    status: ((_a = managerRef.current) == null ? void 0 : _a.getStatus()) || "disconnected",
    retryCount: ((_b = managerRef.current) == null ? void 0 : _b.getRetryCount()) || 0,
    nextRetryDelay: ((_c = managerRef.current) == null ? void 0 : _c.getNextRetryDelay()) || 0,
    ws,
    send,
    close,
    connect: async () => {
      var _a2;
      await ((_a2 = managerRef.current) == null ? void 0 : _a2.connect());
    },
    disconnect: (reason) => {
      var _a2;
      (_a2 = managerRef.current) == null ? void 0 : _a2.disconnect(reason);
    }
  };
}
  ...
  const { status, send, retryCount } = useWebSocketReconnection({
    ...
    onMessage: (event) => {
      try {
        ...
        logger10.error("Failed to parse WebSocket message:", error);
      }
    },
    onConnected: () => {
      logger10.info(`Connected to market data stream for ${symbol}`);
      send(JSON.stringify({
        method: "SUBSCRIBE",
    ...
    onError: (error, retryCount2) => {
      logger10.error(`Stream error (retry ${retryCount2}):`, error);
    },
    onMaxRetriesReached: () => {
      logger10.error("Failed to connect to market data stream after max retries");
    }
  });
  return { status, retryCount };
}
var logger11 = createLogger("StreamStatus");
var statusConfig = {
  connected: {
    color: "#4caf50",
...
var StreamStatus = ({
  ...
  import_react2.default.useEffect(() => {
    logger11.debug(`Status changed to: ${status}`, { symbol, message });
  ...
  const handleRetry = () => {
    logger11.info("Retry connection requested");
  ...
  return (0, import_jsx_runtime.jsxs)(
    "div",
    {
      ...
      children: [
        ...
        !compact && (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
          (0, import_jsx_runtime.jsx)("span", { style: { color: config.color, fontWeight: 500 }, children: config.label }),
          symbol && (0, import_jsx_runtime.jsx)("span", { style: { color: config.color, opacity: 0.8 }, children: symbol }),
          message && (0, import_jsx_runtime.jsx)("span", { style: { color: config.color, opacity: 0.7, fontSize: "12px" }, children: message }),
          (status === "error" || status === "disconnected") && onRetry && (0, import_jsx_runtime.jsx)(
            "button",
            {
              onClick: handleRetry,
              style: {
...
var logger12 = createLogger("StreamingChartWrapper");
var StreamingChartWrapper = ({
  children,
  symbol,
  timeframe,
  enabled = true,
  ...
  const unlistenStatusRef = (0, import_react2.useRef)();
  const unlistenCandleRef = (0, import_react2.useRef)();
  const currentTimeframeRef = (0, import_react2.useRef)(timeframe);
  const connectCandleMonitor = async () => {
    try {
      logger12.debug("Starting candle update monitor...");
      await invoke2("start_candle_monitor", { symbol, timeframe });
      logger12.info("Candle monitor started successfully");
      const updateListener = async () => {
        if (unlistenCandleRef.current) {
          unlistenCandleRef.current();
        }
        const eventName = `market-candles-updated-${currentTimeframeRef.current}`;
        logger12.debug(`Listening for ${eventName} events`);
        unlistenCandleRef.current = await listen2(
          eventName,
          (event) => {
            logger12.debug("Candle update received:", event.payload);
          }
        );
      };
      await updateListener();
      unlistenStatusRef.current = await listen2("market-stream-status", (event) => {
        logger12.debug("Stream status:", event.payload);
      });
    } catch (error) {
      logger12.error("Failed to start market stream:", error);
      throw error;
    }
  };
  const {
    ...
    nextRetryDelay,
    connect: reconnect
  } = useReconnection(connectCandleMonitor, {
    enabled: enabled && !!symbol,
    autoConnect: enabled && !!symbol,
    maxRetries: 10,
    initialDelay: 1e3,
    maxDelay: 3e4,
    backoffMultiplier: 1.5,
    onStatusChange: (status) => {
      logger12.info("Connection status changed:", status);
      onConnectionChange == null ? void 0 : onConnectionChange(status);
    },
    onError: (error, retryCount2) => {
      logger12.error(`Connection error (retry ${retryCount2}):`, error);
    },
    onMaxRetriesReached: () => {
      logger12.error("Failed to connect after maximum retries");
    }
  });
  (0, import_react2.useEffect)(() => {
    currentTimeframeRef.current = timeframe;
    if (connectionStatus === "connected" && unlistenCandleRef.current) {
      const updateListener = async () => {
        if (unlistenCandleRef.current) {
          unlistenCandleRef.current();
        }
        const eventName = `market-candles-updated-${timeframe}`;
        logger12.debug(`Updating listener to ${eventName}`);
        try {
          unlistenCandleRef.current = await listen2(
            eventName,
            (event) => {
          ...
          logger12.error("Failed to update candle listener:", error);
        }
      };
      updateListener();
    }
  ...
  (0, import_react2.useEffect)(() => {
    return () => {
      logger12.debug("Cleaning up streaming wrapper");
      if (unlistenStatusRef.current) unlistenStatusRef.current();
      if (unlistenCandleRef.current) unlistenCandleRef.current();
      invoke2("stop_candle_monitor").catch((error) => {
        logger12.error("Failed to stop candle monitor:", error);
      });
    };
  }, []);
  const connectionMessage = connectionStatus === "reconnecting" && retryCount > 0 ? `Retry ${retryCount} in ${Math.ceil(nextRetryDelay / 1e3)}s` : void 0;
  return (0, import_jsx_runtime.jsxs)("div", { style: { position: "relative", width: "100%", height: "100%" }, children: [
    children,
    enabled && showStatus && (0, import_jsx_runtime.jsx)(
      StreamStatus,
      {
        status: connectionStatus,
        symbol,
...
var logger13 = createLogger("WebSocketService");
var WebSocketService = class {
  ...
  connect() {
    return new Promise((resolve, reject) => {
      logger13.info("Connecting to WebSocket server...", { url: this.config.url });
      this.manager = new WebSocketReconnectionManager(
        ...
        void 0,
        {
          ...
          onDisconnected: (reason) => {
            logger13.warn("WebSocket disconnected", { reason });
            this.isConnected = false;
          },
          onError: (error, retryCount) => {
            logger13.error("WebSocket error", { error, retryCount });
            if (retryCount === 0) {
              reject(error);
            }
          },
  ...
  resubscribeAll() {
    logger13.info("Resubscribing to all active subscriptions", {
  ...
  handleMessage(event) {
    try {
      ...
      logger13.info("Received WebSocket message", { message });
      this.subscriptions.forEach((sub) => {
        if (sub.onRawMessage) {
      ...
      if (message.type === "data" && message.data) {
        ...
        logger13.info("Status update", { message });
      } else if (message.type === "error") {
        logger13.error("Stream error", { error: message.error || message });
      }
    } catch (error) {
      logger13.error("Failed to parse WebSocket message", { error, data: event.data });
    }
  }
  handleCandleUpdate(message) {
    if (!message.symbol || !message.timeframe || !message.data) {
      logger13.warn("Invalid candle update message", { message });
      return;
    }
    ...
    if (subscription) {
      ...
      logger13.trace("Received update for unsubscribed symbol", {
        symbol: message.symbol,
  ...
  subscribe(symbol, timeframe, callback, onRawMessage) {
    ...
    if (this.subscriptions.has(key)) {
      logger13.warn("Already subscribed", { symbol, timeframe });
      return () => {
      };
    }
    logger13.info("Subscribing to stream", { symbol, timeframe });
  ...
  unsubscribe(symbol, timeframe) {
    const key = this.getSubscriptionKey(symbol, timeframe);
    if (!this.subscriptions.has(key)) {
      logger13.warn("Not subscribed", { symbol, timeframe });
      return;
    }
    logger13.info("Unsubscribing from stream", { symbol, timeframe });
  ...
  send(data) {
    if (!this.manager) {
      logger13.error("Cannot send: WebSocket manager not initialized");
      return;
    }
    const message = JSON.stringify(data);
    logger13.trace("Sending message", { data });
  ...
  disconnect() {
    var _a, _b;
    logger13.info("Disconnecting WebSocket");
    this.subscriptions.clear();
    if (this.manager) {
      const ws = (_b = (_a = this.manager).getWebSocket) == null ? void 0 : _b.call(_a);
  ...
  getStatus() {
    var _a;
    return ((_a = this.manager) == null ? void 0 : _a.getStatus()) ?? "disconnected";
  }
  getRetryCount() {
    var _a;
    return ((_a = this.manager) == null ? void 0 : _a.getRetryCount()) ?? 0;
  }
  isConnectedStatus() {
...
var logger14 = createLogger("useWebSocketData");
function useWebSocketData({
  ...
  const [status, setStatus] = (0, import_react2.useState)("disconnected");
  const [retryCount, setRetryCount] = (0, import_react2.useState)(0);
  const [lastUpdate, setLastUpdate] = (0, import_react2.useState)();
  const serviceRef = (0, import_react2.useRef)();
  const unsubscribeRef = (0, import_react2.useRef)(null);
  (0, import_react2.useEffect)(() => {
    if (!enabled) return;
    const initWebSocket = async () => {
      try {
        ...
        unsubscribeRef.current = serviceRef.current.subscribe(
          ...
          timeframe,
          (candle) => {
            logger14.trace("Received candle update", { symbol, timeframe, candle });
            setLastUpdate(candle);
            onUpdate == null ? void 0 : onUpdate(candle);
          },
          onRawMessage
        );
      } catch (error) {
        logger14.error("Failed to connect WebSocket", { error });
        setStatus("failed");
      }
    };
    initWebSocket();
    ...
    return () => {
      ...
      if (serviceRef.current) {
        logger14.info("Disconnecting WebSocket on component unmount");
  ...
  const disconnect = () => {
    var _a;
    (_a = serviceRef.current) == null ? void 0 : _a.disconnect();
...
var logger15 = createLogger("WebSocketTestChart");
var WebSocketTestChart = ({
  symbol,
  timeframe,
  wsConfig,
  showDebugPanel = true,
  ...
  const [debugInfo, setDebugInfo] = (0, import_react2.useState)({
    ticksReceived: 0,
    candlesFormed: 0,
    rawMessages: []
  });
  const seriesRef = (0, import_react2.useRef)(null);
  const lastCandleTimeRef = (0, import_react2.useRef)(0);
  const currentCandleRef = (0, import_react2.useRef)(null);
  const candlePeriodRef = (0, import_react2.useRef)(60);
  (0, import_react2.useEffect)(() => {
    const periods = {
      "1m": 60,
      "5m": 300,
    ...
    candlePeriodRef.current = periods[timeframe] || 60;
  }, [timeframe]);
  const processTick = (tick) => {
    const candleTime = Math.floor(tick.time / candlePeriodRef.current) * candlePeriodRef.current;
    logger15.info("Processing tick", {
      tickTime: new Date(tick.time * 1e3).toISOString(),
      tickPrice: tick.price,
      candleTime: new Date(candleTime * 1e3).toISOString(),
      currentCandleTime: currentCandleRef.current ? new Date(currentCandleRef.current.time * 1e3).toISOString() : "none",
      willCreateNewCandle: !currentCandleRef.current || currentCandleRef.current.time !== candleTime
    });
    if (!currentCandleRef.current || currentCandleRef.current.time !== candleTime) {
      if (currentCandleRef.current) {
        const completedCandle = { ...currentCandleRef.current };
        logger15.info("Candle completed", completedCandle);
        setDebugInfo((prev) => ({
          ...prev,
          candlesFormed: prev.candlesFormed + 1
        }));
      }
      currentCandleRef.current = {
        time: Math.floor(candleTime),
        // Ensure it's a number and treated as UTCTimestamp
        open: tick.price,
        high: tick.price,
        low: tick.price,
        close: tick.price
    ...
    setDebugInfo((prev) => ({
      ...prev,
      currentCandle: currentCandleRef.current ? { ...currentCandleRef.current } : void 0
    }));
    if (seriesRef.current && currentCandleRef.current) {
      try {
        if (currentCandleRef.current.time >= lastCandleTimeRef.current) {
          seriesRef.current.update(currentCandleRef.current);
          lastCandleTimeRef.current = currentCandleRef.current.time;
        } else {
          logger15.warn("Skipping out-of-order candle update", {
            currentTime: currentCandleRef.current.time,
            lastTime: lastCandleTimeRef.current
          });
        }
      } catch (error) {
        logger15.error("Failed to update chart", {
          error,
          candleTime: currentCandleRef.current.time,
          candleData: currentCandleRef.current
        });
      }
    }
  };
  const { status, retryCount, connect, disconnect } = useWebSocketData({
    symbol,
    timeframe,
    wsConfig,
    enabled: visible,
    onUpdate: (candle) => {
      logger15.info("Received processed candle", candle);
    },
  ...
  const handleWebSocketMessage = (data) => {
    var _a, _b;
    try {
      logger15.info("WebSocket data received", data);
      setDebugInfo((prev) => ({
        ...prev,
        rawMessages: [...prev.rawMessages.slice(-9), JSON.stringify(data, null, 2)],
        ticksReceived: prev.ticksReceived + 1
      }));
      if (data.type === "tick" || data.data && data.data.type === "tick") {
        const tickData = data.data || data;
        let timestamp = tickData.timestamp || tickData.time || Date.now();
        if (timestamp > 9999999999) {
          timestamp = timestamp / 1e3;
        }
        const tick = {
          time: timestamp,
          price: tickData.price || ((_a = tickData.data) == null ? void 0 : _a.price),
          volume: tickData.volume || ((_b = tickData.data) == null ? void 0 : _b.volume)
        };
        if (tick.price) {
          setDebugInfo((prev) => ({
            ...prev,
      ...
      logger15.error("Error processing WebSocket data", error);
    }
  };
  return (0, import_jsx_runtime.jsxs)("div", { style: { display: "flex", height: "100%", gap: "10px" }, children: [
    (0, import_jsx_runtime.jsx)("div", { style: { flex: showDebugPanel ? "1 1 60%" : "1 1 100%" }, children: (0, import_jsx_runtime.jsx)(
      MarketDataChart,
      {
        symbol,
        timeframe
    ...
    showDebugPanel && (0, import_jsx_runtime.jsxs)("div", { style: {
      flex: "0 0 40%",
      ...
      (0, import_jsx_runtime.jsx)("h3", { children: "WebSocket Debug Info" }),
      (0, import_jsx_runtime.jsxs)("div", { style: { marginBottom: "20px" }, children: [
        (0, import_jsx_runtime.jsx)("h4", { children: "Connection" }),
        (0, import_jsx_runtime.jsxs)("div", { children: [
          "Status: ",
          (0, import_jsx_runtime.jsx)("span", { style: { color: status === "connected" ? "#4caf50" : "#f44336" }, children: status })
        ] }),
        (0, import_jsx_runtime.jsxs)("div", { children: [
          "Retry Count: ",
          retryCount
      ...
      (0, import_jsx_runtime.jsxs)("div", { style: { marginBottom: "20px" }, children: [
        (0, import_jsx_runtime.jsx)("h4", { children: "Statistics" }),
        (0, import_jsx_runtime.jsxs)("div", { children: [
          "Ticks Received: ",
          debugInfo.ticksReceived
        ] }),
        (0, import_jsx_runtime.jsxs)("div", { children: [
          "Candles Formed: ",
          debugInfo.candlesFormed
        ] })
      ] }),
      debugInfo.lastTick && (0, import_jsx_runtime.jsxs)("div", { style: { marginBottom: "20px" }, children: [
        (0, import_jsx_runtime.jsx)("h4", { children: "Last Tick" }),
        (0, import_jsx_runtime.jsxs)("div", { children: [
          "Time: ",
          new Date(debugInfo.lastTick.time * 1e3).toLocaleString()
        ] }),
        (0, import_jsx_runtime.jsxs)("div", { children: [
          "Price: ",
          debugInfo.lastTick.price
        ] }),
        (0, import_jsx_runtime.jsxs)("div", { children: [
          "Volume: ",
          debugInfo.lastTick.volume || "N/A"
        ] })
      ] }),
      debugInfo.currentCandle && (0, import_jsx_runtime.jsxs)("div", { style: { marginBottom: "20px" }, children: [
        (0, import_jsx_runtime.jsx)("h4", { children: "Current Forming Candle" }),
        (0, import_jsx_runtime.jsxs)("div", { children: [
          "Time: ",
          new Date(debugInfo.currentCandle.time * 1e3).toLocaleString()
        ] }),
        (0, import_jsx_runtime.jsxs)("div", { children: [
          "Open: ",
          debugInfo.currentCandle.open
        ] }),
        (0, import_jsx_runtime.jsxs)("div", { children: [
          "High: ",
          debugInfo.currentCandle.high
        ] }),
        (0, import_jsx_runtime.jsxs)("div", { children: [
          "Low: ",
          debugInfo.currentCandle.low
        ] }),
        (0, import_jsx_runtime.jsxs)("div", { children: [
          "Close: ",
          debugInfo.currentCandle.close
        ] })
      ] }),
      (0, import_jsx_runtime.jsxs)("div", { style: { marginBottom: "20px" }, children: [
        (0, import_jsx_runtime.jsx)("h4", { children: "Actions" }),
        (0, import_jsx_runtime.jsx)("button", { onClick: connect, style: { marginRight: "10px" }, children: "Reconnect" }),
        (0, import_jsx_runtime.jsx)("button", { onClick: disconnect, children: "Disconnect" })
      ] }),
      (0, import_jsx_runtime.jsxs)("div", { children: [
        (0, import_jsx_runtime.jsx)("h4", { children: "Raw Messages (Last 10)" }),
        (0, import_jsx_runtime.jsx)("div", { style: {
          ...
          maxHeight: "400px",
          overflow: "auto"
        }, children: debugInfo.rawMessages.map((msg, idx) => (0, import_jsx_runtime.jsx)("pre", { style: {
...
function useCountdownTimer(timeframe, options) {
  ...
  const [countdown, setCountdown] = (0, import_react2.useState)("00:00");
  const [countdownColor, setCountdownColor] = (0, import_react2.useState)("#999");
  const [secondsRemaining, setSecondsRemaining] = (0, import_react2.useState)(0);
  const intervalRef = (0, import_react2.useRef)(null);
  const lastUpdateRef = (0, import_react2.useRef)(0);
  const hasTriggeredRef = (0, import_react2.useRef)(false);
  const calculateSecondsToNextCandle = (0, import_react2.useCallback)((timeframe2) => {
    const now = /* @__PURE__ */ new Date();
    const seconds2 = now.getSeconds();
    const minutes2 = now.getMinutes();
    const hours2 = now.getHours();
  ...
  const formatCountdown = (0, import_react2.useCallback)((totalSeconds) => {
    const displaySeconds = Math.max(0, totalSeconds);
    const minutes2 = Math.floor(displaySeconds / 60);
    const seconds2 = displaySeconds % 60;
    return `${minutes2.toString().padStart(2, "0")}:${seconds2.toString().padStart(2, "0")}`;
  }, []);
  const getCountdownColor = (0, import_react2.useCallback)((seconds2) => {
    if (seconds2 <= criticalThreshold) {
      return "#ffae00";
    } else if (seconds2 <= warningThreshold) {
      return "#ccc";
    } else {
      return "#999";
    }
  }, [criticalThreshold, warningThreshold]);
  const updateCountdown = (0, import_react2.useCallback)(() => {
    const now = Date.now();
    const seconds2 = Math.floor(now / 1e3) % 60;
    const nearBoundary = seconds2 >= 58 || seconds2 <= 2;
    ...
    if (totalSeconds <= 0 && !hasTriggeredRef.current) {
      ...
      onNewCandleBoundary == null ? void 0 : onNewCandleBoundary(candleTime);
    }
    if (totalSeconds > 5) {
      hasTriggeredRef.current = false;
    }
...
var CountdownTimer = ({
  ...
  const positionStyles = import_react2.default.useMemo(() => {
    const styles = {
      position: "absolute",
  ...
  return (0, import_jsx_runtime.jsx)(Box, { style: positionStyles, children: (0, import_jsx_runtime.jsxs)("div", { style: { display: "flex", alignItems: "center", gap: "6px" }, children: [
    (0, import_jsx_runtime.jsx)("span", { style: { color: "#666", fontSize: "11px" }, children: "Next:" }),
...
var DetailedCountdownTimer = ({
  ...
  return (0, import_jsx_runtime.jsxs)(Box, { style: positionStyles, children: [
    showTimeframe && (0, import_jsx_runtime.jsx)(Text, { size: "xs", color: "dimmed", style: { marginBottom: 4 }, children: timeframe.toUpperCase() }),
    (0, import_jsx_runtime.jsx)(
      Text,
      {
        size: "sm",
    ...
    isPastBoundary && (0, import_jsx_runtime.jsx)(Text, { size: "xs", color: "yellow", style: { marginTop: 4 }, children: "New candle!" })
  ] });
};
var TimeframeWatermark = ({
  timeframe,
  position = "center",
  customPosition,
  opacity = 0.08,
  fontSize = "min(10vw, 72px)",
  color: color2 = "rgba(255, 255, 255, opacity)"
}) => {
  ...
  const positionStyles = {
    ...
    "top-right": {
      top: "60px",
      // Below other UI elements
      right: "20px"
    },
    "axis-center": {
      bottom: "5px",
      // Inside the time axis area
      left: "50%",
      transform: "translateX(-50%)"
    },
    "axis-right": {
      ...
      // Inside the time axis area
      right: "65px"
      // Account for price axis width
    },
    custom: customPosition || {}
  };
  const displayTimeframe = timeframe || timeframe;
  const baseTransform = "transform" in positionStyles[position] ? positionStyles[position].transform : "";
  return (0, import_jsx_runtime.jsx)(
    "div",
    {
      style: {
        ...
        fontSize,
        fontWeight: "700",
        color: color2.replace("opacity", opacity.toString()),
        pointerEvents: "none",
        userSelect: "none",
        fontFamily: "system-ui, -apple-system, sans-serif",
        letterSpacing: "0.1em",
        textTransform: "uppercase",
        transition: isAnimating ? "all 300ms cubic-bezier(0.4, 0.0, 0.2, 1)" : "opacity 200ms ease-out",
        opacity: isAnimating ? Math.min(opacity * 1.8, 0.35) : opacity,
        // Subtle flash: 1.8x brighter
        transform: isAnimating ? `${baseTransform} scale(1.015)`.trim() : baseTransform || "none",
        textShadow: isAnimating ? `0 0 10px rgba(255, 255, 255, ${opacity * 1.2})` : "none",
...
var MinimalTimeframeWatermark = ({ timeframe }) => (0, import_jsx_runtime.jsx)(
  TimeframeWatermark,
  {
    timeframe,
    opacity: 0.05,
...
var logger16 = createLogger("useTradingStore");
var useTradingStore = create()(
  devtools(
    persist(
      (set) => ({
        ...
        setPair: (pair) => {
          logger16.debug("setPair called with:", pair);
          set((state) => {
            logger16.debug("Current selectedPair:", state.selectedPair);
            logger16.debug("New selectedPair will be:", pair);
            return { selectedPair: pair };
          });
        },
        setTimeframe: (timeframe) => {
          logger16.debug("setTimeframe:", timeframe);
          set({ selectedTimeframe: timeframe });
        },
        setChartType: (chartType) => {
          logger16.debug("setChartType:", chartType);
          set({ chartType });
        },
        setChartVersion: (chartVersion) => {
          logger16.debug("setChartVersion:", chartVersion);
          set({ chartVersion });
        },
        toggleIndicator: (indicator) => {
          logger16.debug("toggleIndicator:", indicator);
          set((state) => ({
            indicators: {
              ...state.indicators,
...
init_core();
init_event();
var logger17 = createLogger("PairSelector");
var PairSelector = () => {
  const { selectedPair, setPair } = useTradingStore();
  const [isLoading, setIsLoading] = (0, import_react2.useState)(true);
  const [symbols, setSymbols] = (0, import_react2.useState)([]);
  (0, import_react2.useEffect)(() => {
    const loadSymbols = async () => {
      try {
        setIsLoading(true);
        const available = await invoke("get_all_available_symbols");
        ...
        logger17.error("Failed to load symbols:", error);
        setSymbols([]);
      } finally {
        setIsLoading(false);
      }
  ...
  const handlePairChange = (value) => {
    logger17.debug("onChange triggered with value:", value);
    if (value) {
      logger17.debug("Calling setPair with:", value);
      setPair(value);
    }
  };
  return (0, import_jsx_runtime.jsx)(
...
var logger18 = createLogger("useChartSetup");
var darkTheme = {
...
function useChartSetup(containerRef, options) {
  const chartRef = (0, import_react2.useRef)(null);
  const seriesRef = (0, import_react2.useRef)(null);
  const isReadyRef = (0, import_react2.useRef)(false);
  const theme = (options == null ? void 0 : options.theme) || darkTheme;
  const chartOptions = (options == null ? void 0 : options.chartOptions) || {};
  const seriesOptions = (options == null ? void 0 : options.seriesOptions) || {};
  (0, import_react2.useEffect)(() => {
    if (!containerRef.current) return;
    logger18.debug("Creating chart");
    ...
    logger18.debug("Chart created successfully");
    return () => {
      logger18.debug("Cleaning up chart");
...
var logger19 = createLogger("useChartZoom");
function useChartZoom(chart, options) {
  const [isShiftPressed, setIsShiftPressed] = (0, import_react2.useState)(false);
  const [lockedLeftEdge, setLockedLeftEdge] = (0, import_react2.useState)(null);
  const [visibleRange, setVisibleRange] = (0, import_react2.useState)(null);
  const [barSpacing, setBarSpacing] = (0, import_react2.useState)(12);
  const barSpacingCheckInterval = (options == null ? void 0 : options.barSpacingCheckInterval) || 100;
  const lastBarSpacingRef = (0, import_react2.useRef)(12);
  (0, import_react2.useEffect)(() => {
    const handleKeyDown = (e) => {
      if (e.key === "Shift" && !isShiftPressed) {
        setIsShiftPressed(true);
    ...
    const handleKeyUp = (e) => {
      if (e.key === "Shift" && isShiftPressed) {
        ...
        logger19.debug("Left edge lock released");
        if (chart) {
          chart.timeScale().applyOptions({
            rightBarStaysOnScroll: true
          });
  ...
  (0, import_react2.useEffect)(() => {
    ...
    const handleVisibleRangeChange = () => {
      ...
      if (range) {
        ...
        (_a = options == null ? void 0 : options.onVisibleRangeChange) == null ? void 0 : _a.call(options, newRange);
      } else {
        setVisibleRange(null);
        (_b = options == null ? void 0 : options.onVisibleRangeChange) == null ? void 0 : _b.call(options, null);
      }
    };
    ...
    const checkBarSpacing = () => {
      ...
      try {
        ...
        if (currentBarSpacing !== lastBarSpacingRef.current) {
          ...
          (_a = options == null ? void 0 : options.onBarSpacingChange) == null ? void 0 : _a.call(options, currentBarSpacing);
        }
      } catch (e) {
        logger19.error("Error checking bar spacing:", e);
      }
    };
    const intervalId = setInterval(checkBarSpacing, barSpacingCheckInterval);
    return () => clearInterval(intervalId);
  }, [chart, barSpacingCheckInterval, options]);
  const zoomIn = (0, import_react2.useCallback)((factor = 1.2) => {
    if (!chart) return;
  ...
  const zoomOut = (0, import_react2.useCallback)((factor = 1.2) => {
    if (!chart) return;
    const timeScale = chart.timeScale();
    const currentBarSpacing = timeScale.options().barSpacing;
  ...
  const resetZoom = (0, import_react2.useCallback)(() => {
    if (!chart) return;
    chart.timeScale().fitContent();
  }, [chart]);
  const scrollToTime2 = (0, import_react2.useCallback)((time, animate = true) => {
  ...
  const setVisibleRangeCallback = (0, import_react2.useCallback)((range) => {
    if (!chart) return;
    chart.timeScale().setVisibleRange({
      from: range.from,
      to: range.to
  ...
  const maintainLeftEdgeLock = (0, import_react2.useCallback)(() => {
...
var logger20 = createLogger("useChartData");
function useChartData(symbol, timeframe, options) {
  const [data, setData] = (0, import_react2.useState)([]);
  const [isLoading, setIsLoading] = (0, import_react2.useState)(false);
  const [error, setError] = (0, import_react2.useState)(null);
  const currentRef = (0, import_react2.useRef)({ symbol, timeframe });
  const fetchData = (0, import_react2.useCallback)(async (fetchOptions) => {
    if (isLoading) {
      logger20.trace("Already loading, skipping fetch");
      return;
    }
    setIsLoading(true);
    setError(null);
    try {
      const result = await chartDataCoordinator.fetchChartData(
        ...
        timeframe,
        {
          forceRefresh: fetchOptions == null ? void 0 : fetchOptions.forceRefresh,
          range: (fetchOptions == null ? void 0 : fetchOptions.range) || (options == null ? void 0 : options.range)
        }
      );
      if (currentRef.current.symbol === symbol && currentRef.current.timeframe === timeframe) {
        setData(result);
        logger20.debug(`Updated data for ${symbol}-${timeframe}: ${result.length} candles`);
      }
    } catch (err) {
      logger20.error("Fetch error:", err);
      if (currentRef.current.symbol === symbol && currentRef.current.timeframe === timeframe) {
  ...
  }, [symbol, timeframe, isLoading, options == null ? void 0 : options.range]);
  const setDefaultRange = (0, import_react2.useCallback)((from, to) => {
    chartDataCoordinator.setDefaultRange(symbol, timeframe, from, to);
  }, [symbol, timeframe]);
  const invalidateCache = (0, import_react2.useCallback)((pattern) => {
    chartDataCoordinator.invalidateCache(pattern);
  }, []);
  (0, import_react2.useEffect)(() => {
    currentRef.current = { symbol, timeframe };
    setData([]);
    if ((options == null ? void 0 : options.autoLoad) !== false) {
      fetchData();
    }
  }, [symbol, timeframe, options == null ? void 0 : options.autoLoad]);
  (0, import_react2.useEffect)(() => {
    return () => {
      logger20.trace(`Unmounting for ${symbol}-${timeframe}`);
    };
  }, [symbol, timeframe]);
  return {
    data,
...
var MarketChartPage = ({
  ...
  const chartTimeframe = timeframe || selectedTimeframe || "1h";
  return (0, import_jsx_runtime.jsx)(Box, { style: { height: "100%", width: "100%", background: "#0a0a0a", position: "relative" }, children: (0, import_jsx_runtime.jsx)(
...
export {
  ...
  LogLevel,
  MarketChartPage,
  MarketDataChart_default as MarketDataChart,
  MinimalStreamStatus,
  MinimalTimeframeWatermark,
  NoDataInRangeState,
  PairSelector,
  PriceScaleMode,
  ReconnectionManager,
  StreamStatus,
  StreamingChartWrapper,
  TIMEFRAME_SECONDS,
  TIMEFRAME_SWITCH_THRESHOLDS,
  TimeframeWatermark,
  TimeframeWatermarkPrimitive,
...
lightweight-charts/dist/lightweight-charts.development.mjs:
   ...
   *)
*/
//# sourceMappingURL=sptrader-chart-lib.js.map
