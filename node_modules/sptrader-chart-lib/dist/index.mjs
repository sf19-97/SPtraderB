import React3, { useRef, useCallback, Component, useState, useEffect } from 'react';
import { createChart } from 'lightweight-charts';
export { CrosshairMode, LineStyle, PriceScaleMode, createChart, isBusinessDay, isUTCTimestamp } from 'lightweight-charts';
...
function usePlaceholderCandle(series, options) {
  ...
  const placeholderTimeRef = useRef(null);
  const resetTimeoutRef = useRef(null);
  const createPlaceholder = useCallback((candleTime) => {
  ...
  const updateWithRealData = useCallback((data) => {
    ...
    if (hasError && error) {
      ...
      return /* @__PURE__ */ jsxs(
        "div",
        {
          ...
          children: [
            /* @__PURE__ */ jsx("h2", { style: { color: "#c92a2a", marginTop: 0 }, children: "Something went wrong" }),
            /* @__PURE__ */ jsxs("details", { style: { whiteSpace: "pre-wrap", marginBottom: "20px" }, children: [
              /* @__PURE__ */ jsx("summary", { style: { cursor: "pointer", color: "#666" }, children: "Error details" }),
              /* @__PURE__ */ jsx("p", { style: { color: "#666", marginTop: "10px" }, children: error.toString() }),
              errorInfo && /* @__PURE__ */ jsx("pre", { style: { color: "#666", fontSize: "12px", overflow: "auto" }, children: errorInfo.componentStack })
            ] }),
            /* @__PURE__ */ jsx(
              "button",
              {
...
var ChartErrorBoundary = ({
  ...
  return /* @__PURE__ */ jsx(
    ...
    {
      onError,
      fallback: (error, _errorInfo, retry) => /* @__PURE__ */ jsxs(
        "div",
        {
          style: {
            display: "flex",
      ...
      children: [
        ...
        /* @__PURE__ */ jsx(
          "div",
          {
            ...
            children: /* @__PURE__ */ jsx("div", { style: { display: "flex", justifyContent: "space-around", height: "100%", alignItems: "flex-end" }, children: [...Array(8)].map((_, i) => /* @__PURE__ */ jsx(
              "div",
              {
                style: {
                  width: "8px",
  ...
  if (!symbol) {
    return /* @__PURE__ */ jsx(
      EmptyState,
      {
        ...
        icon: /* @__PURE__ */ jsx("div", { style: { fontSize: "48px", marginBottom: "20px", color: "#dee2e6" }, children: "\u{1F50D}" }),
...
var NoDataInRangeState = ({ timeframe, onChangeTimeframe }) => {
  return /* @__PURE__ */ jsx(
    EmptyState,
    {
      ...
      icon: /* @__PURE__ */ jsx("div", { style: { fontSize: "48px", marginBottom: "20px", color: "#dee2e6" }, children: "\u{1F4C5}" }),
...
var MarketDataChart = ({
  ...
  const lockedLeftEdgeRef = useRef(null);
  ...
  if (isLoading && !initialLoadDoneRef.current) {
    return /* @__PURE__ */ jsx(ChartErrorBoundary, { children: /* @__PURE__ */ jsx(Box, { style: { position: "relative", width: "100%", height: "100%" }, children: /* @__PURE__ */ jsx(ChartSkeleton, { height: "100%" }) }) });
  }
  if (!symbol) {
    return /* @__PURE__ */ jsx(ChartErrorBoundary, { children: /* @__PURE__ */ jsx(Box, { style: { position: "relative", width: "100%", height: "100%" }, children: /* @__PURE__ */ jsx(ChartEmptyState, {}) }) });
  }
  return /* @__PURE__ */ jsx(ChartErrorBoundary, { onError: (err) => setError(err), children: /* @__PURE__ */ jsx(Box, { style: { position: "relative", width: "100%", height: "100%" }, children: /* @__PURE__ */ jsxs(
    "div",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxs(
          "div",
          {
            ...
            children: [
              currentTimeframe,
              isShiftPressed && /* @__PURE__ */ jsx("span", { style: { marginLeft: "10px", color: "#ff9900" }, children: "[LOCK LEFT]" })
            ]
          }
        )
      ]
    ...
    "div",
    {
      ...
      children: [
        ...
        !compact && /* @__PURE__ */ jsxs(Fragment, { children: [
          /* @__PURE__ */ jsx("span", { style: { color: config.color, fontWeight: 500 }, children: config.label }),
          symbol && /* @__PURE__ */ jsx("span", { style: { color: config.color, opacity: 0.8 }, children: symbol }),
          message && /* @__PURE__ */ jsx("span", { style: { color: config.color, opacity: 0.7, fontSize: "12px" }, children: message }),
          (status === "error" || status === "disconnected") && onRetry && /* @__PURE__ */ jsx(
            "button",
            {
              onClick: handleRetry,
              style: {
...
function useChartZoom(chart, options) {
  const [isShiftPressed, setIsShiftPressed] = useState(false);
  const [lockedLeftEdge, setLockedLeftEdge] = useState(null);
  ...
  const scrollToTime2 = useCallback((time, animate = true) => {
  ...
  const setVisibleRangeCallback = useCallback((range) => {
    if (!chart) return;
    chart.timeScale().setVisibleRange({
      from: range.from,
      to: range.to
...
function useChartData(symbol, timeframe, options) {
  ...
  const setDefaultRange = useCallback((from, to) => {
...
function useCountdownTimer(timeframe, options) {
  ...
  const [countdownColor, setCountdownColor] = useState("#999");
  const [secondsRemaining, setSecondsRemaining] = useState(0);
  const intervalRef = useRef(null);
  const lastUpdateRef = useRef(0);
  const hasTriggeredRef = useRef(false);
  const calculateSecondsToNextCandle = useCallback((timeframe2) => {
    const now = /* @__PURE__ */ new Date();
    const seconds = now.getUTCSeconds();
    const minutes = now.getUTCMinutes();
    const hours = now.getUTCHours();
...
var useTradingStore = create((set) => ({
  selectedPair: "EURUSD",
...
var MarketChartPage = ({
  ...
  return /* @__PURE__ */ jsx(Box, { style: { height: "100%", width: "100%", background: "#0a0a0a", position: "relative" }, children: /* @__PURE__ */ jsx(
...
export { ADJACENT_TIMEFRAMES, ALL_TIMEFRAMES, ChartDataCoordinator, ChartEmptyState, ChartErrorBoundary, ChartLoadingState, ChartSkeleton, CountdownTimerPrimitive, DEFAULT_TIMEFRAME, EmptyState, ErrorBoundary, HTTPDataProvider, InlineLoader, LoadingState, LogLevel, MarketChartPage, MarketDataChart_default as MarketDataChart, MinimalStreamStatus, NoDataInRangeState, ReconnectionManager, StreamStatus, TIMEFRAME_SECONDS, TIMEFRAME_SWITCH_THRESHOLDS, TimeframeWatermarkPrimitive, WebSocketReconnectionManager, WebSocketService, calculateBarSpacingForTimeframeSwitch, calculateCandleTime, chartDataCoordinator, createLogger, createPlaceholderCandle, darkTheme, defaultChartOptions, defaultSeriesOptions, destroyHTTPDataProvider, destroyWebSocketService, findLastRealCandle, getChartRequestStats, getCurrentCandleTime, getDaysToShowForTimeframe, getHTTPDataProvider, getMetricsSummary, getNextCandleTime, getTimeframePeriodSeconds, getVisibleRangeDuration, getWebSocketService, invoke2 as invoke, isTimeVisible, listen2 as listen, logger, mockInvoke, mockListen, printMetricsSummary, resetChartRequestStats, resetMetrics, scrollToTime, setVisibleRangeByDays, useChartData, useChartDataStream, useChartSetup, useChartStore, useChartZoom, useCountdownTimer, usePlaceholderCandle, useReconnection, useTradingStore, useWebSocketData, useWebSocketReconnection };
